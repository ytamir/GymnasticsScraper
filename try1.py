...
updategymnasttable = True
if(updategymnasttable):
# Parse Gymnasts Table
    datatable = []
    count = 0
    testtable = ['https://thegymter.net/vasiliki-millousi/','https://thegymter.net/maria-simou/']
...
updateScoresTable = False
if(updateScoresTable):
# Parse Gymnasts Table
    datatable = []
    count = 0
    ...
    for gymnastlink in removedTopBottomLinks:
        ...
        # Find the first table 
        tables = tablesoup.find_all('table')
        strongs = tablesoup.find_all('strong')
        years= []
        #print(strongs)
        for strong in strongs:
           # print("*********************************************")
            ...
            #print(strong)
            if (re.search("20[1-9][1-9]",strong)):
                strong = strong.strip()
                years.append(strong[:4])
        #print("years")
        #print(years)
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if(len(cols) == 8 ):
                    if (notfirst):
                        cols.insert(0, count)
                        cols.insert(0, fullname)

                        #reduce name to 1
                        if("-" in cols[2] ):
                            index = cols[2].find("-")
                            cols[2] = cols[2][:index]
                        cols[2] = cols[2]+", "+years[tablecount]
                        cols[2] = cols[2].replace(u'\xa0', u' ')    
                        data = cols
                        #print(cols)
                        #print(len(cols))
                        # save event if it needs to be repeated
                        last = cols
                    else:
                        notfirst = True
                # not new listing
                    ...
                    if (notfirst):
                       # print("last")
                       # print(last)
                        cols.insert(0, last[3])
                        cols.insert(0, last[2])
                        cols.insert(0, count)
                        cols.insert(0, fullname)
                ...
                data2 = []
                for item in data:
                    data2.append(str(item).replace("*", ""))
                datatable.append(data2)
                data2 = []
                cols = []
                data = []
            tablecount += 1
        count+=1
    datatable1 = [x for x in datatable if x]
    #for a in datatable1:
        #print(a)
        #print(len(a))
   # print("datatable1")
...
if(page2):
    ...
    df = pd.DataFrame(np.array(datatable1),
                        columns=['Full Name', 'id', 'date', 'Event', 'Round', 'VT', 'UB', 'BB', 'FX', 'AA'])
    print(df)
...
if(page1):
    ...
    df = pd.DataFrame(np.array(datatable),
                        columns=['id', 'Full Name', 'Nation', 'DOB', 'Status'])
    print(df)
